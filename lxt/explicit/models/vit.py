...
class ViTEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if bool_masked_pos is not None:
            ...
            embeddings = lf.add2(embeddings * (1.0 - mask), mask_tokens * mask)
        ...
        if interpolate_pos_encoding:
            embeddings = lf.add2(embeddings, self.interpolate_pos_encoding(embeddings, height, width))
        else:
            embeddings = lf.add2(embeddings, self.position_embeddings)
...
class ViTOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = lf.add2(hidden_states, input_tensor)
...
class ViTLayer(nn.Module):
    ...
    def __init__(self, config: ViTConfig) -> None:
        ...
        self.layernorm_before = lm.LayerNormEpsilon(config.hidden_size, eps=config.layer_norm_eps)
        self.layernorm_after = lm.LayerNormEpsilon(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(
        ...
        hidden_states = lf.add2(attention_output, hidden_states)
