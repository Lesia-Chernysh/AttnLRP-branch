...
import transformers.models.vilt.modeling_vilt as modeling_vilt

from lxt.efficient.rules import divide_gradient, identity_rule_implicit
from lxt.efficient.patches import patch_method, replace_module, layer_norm_forward, dropout_forward, non_linear_forward
...
attnLRP = {
    ...
    modeling_vilt: partial(replace_module, sys.modules[__name__]),
}

zennit_composite = LayerMapComposite([
    (torch.nn.Conv2d, z_rules.Gamma(10)),
    (torch.nn.Linear, z_rules.Gamma(0.1)),
])

# modified code is commented with `### <------------------------------------------- LXT`
##########################################################################
...
class ViltSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        attention_scores = divide_gradient(attention_scores, 2) ### <------------------------------------------- LXT
        ...
        context_layer = divide_gradient(context_layer, 2) ### <------------------------------------------- LXT
...
class ViltIntermediate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = identity_rule_implicit(self.intermediate_act_fn, hidden_states) ### <------------------------------------------- LXT
        return hidden_states
...
class ViltPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        pooled_output = identity_rule_implicit(self.activation, pooled_output) ### <------------------------------------------- LXT
...
class ViltPredictionHeadTransform(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_states = self.dense(hidden_states)
        hidden_states = identity_rule_implicit(self.transform_act_fn, hidden_states) ### <------------------------------------------- LXT
